...
import { getUploadedImages } from "../create/helpers/getUploadedImages";
import { getUploadedVideos } from "../create/helpers/getUploadedVideos";
import { deleteFiles } from "../create/helpers/deleteFiles";
...
export const createTestimonial = async (req: Request, res: Response) => {
    try {
        ...
        const testimonialCoreResult = await supabase.schema("testimonials").from("testimonials").insert({...testimonialCoreData, testimonial_id : testimonialId});
        ReturnError(res, "Error inserting testimonial core data into Supabase", testimonialCoreResult.error);

        const testimonialMediaUploadResult = await supabase.schema("testimonials").from("testimonial_media").insert({...testimonialMediaData, testimonial_id : testimonialId});
        ReturnError(res, "Error inserting testimonial media data into Supabase", testimonialMediaUploadResult.error);

        const testimonialActivityResult = await supabase.schema("testimonials").from("testimonial_activities").insert({activities : activites, testimonial_id : testimonialId});
        ReturnError(res, "Error inserting testimonial activity data into Supabase", testimonialActivityResult.error);

        const testimonialTagsResult = await supabase.schema("testimonials").from("testimonial_tags").insert({travel_tags : travel_tags, testimonial_id : testimonialId});
