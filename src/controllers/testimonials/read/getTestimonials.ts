...
export const getTestimonials = async (req: Request, res: Response) => {
    try {
        ...
        const { count } = await supabase.schema("testimonials").from("testimonials").select("id", { count : "exact" });
        let query = supabase.schema("testimonials").from("testimonials").select("*, testimonial_media(photos, videos), testimonial_activities(activities), testimonial_tags(travel_tags)");
