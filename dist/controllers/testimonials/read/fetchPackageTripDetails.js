...
const fetchPackageTripDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        try {
            ...
            const { package_id, total_duration, destinations: { destinations }, travel_tags, vendor_id } = response.data.data.general;
            const accomodations = response.data.data.accommodations[0].type_of_stay;
            const transportation = [...new Set((_a = response === null || response === void 0 ? void 0 : response.data) === null || _a === void 0 ? void 0 : _a.data.transfers.map((t) => t.mode_of_transport))].join(', ');
        ...
        try {
            const tripSupabaseResponse = yield supabase_1.supabase.schema("my_trips").from("base_trips").select("trip_id, number_of_person").eq("trip_id", tripId).single();
            if (!tripSupabaseResponse.data || tripSupabaseResponse.status !== 200) {
                return (0, apiResponse_1.apiErrorResponse)({ res, success: false, message: "Error in fetching trip details", reason: tripSupabaseResponse.error });
            }
            let travel_type = "";
